...
reqFailureUnitTests = testGroup "reqFailureUnitTests" $
  ...
  [ testCase "oneSkillAndxsymbol_permittedApply_FailureApplyFood" $
      permittedApply timeZero 1 True Nothing testItemFull quantSingle 
      @?= Left ApplyFood
  , testCase "oneSkillAnd,symbolAndCGround_permittedApply_True" $
      permittedApply timeZero 1 True (Just CGround) testItemFull {itemKind=testItemKind{isymbol=','}}  quantSingle 
      @?= Right True
  , testCase "oneSkillAnd\"symbol_permittedApply_True" $
      permittedApply timeZero 1 True Nothing testItemFull {itemKind=testItemKind{isymbol='"'}}  quantSingle 
      @?= Right True
  , testCase "twoSkillAnd?symbol_permittedApply_FailureApplyRead" $
      permittedApply timeZero 2 True Nothing testItemFull {itemKind=testItemKind{isymbol='?'}}  quantSingle 
      @?= Left ApplyRead 
  ]