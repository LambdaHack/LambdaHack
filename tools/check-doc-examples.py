...
types branch there are live positive controls worth running: on the three
documents with fenced Haskell it should extract and resolve 32, 12 and 6
capitalised names (leader-desync-bug, promptgetkey-hygiene,
wasm-frontend-unified-plan) with none unresolved, the count being the
thing to check, since "0 unresolved" reads the same whether the extractor
works or finds nothing.
