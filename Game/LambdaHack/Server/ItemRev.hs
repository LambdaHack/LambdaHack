...
buildItem :: FlavourMap -> DiscoveryKindRev -> Kind.Id ItemKind -> ItemKind
          -> LevelId -> Dice.Dice
          -> Item
buildItem (FlavourMap flavour) discoRev ikChosen kind lid jdamage =
  let jkindIx  = discoRev EM.! ikChosen
      jsource  = ItemSourceLevel lid  -- the default
...
newItem Kind.COps{coitem=Kind.Ops{ofoldlGroup'}}
  ...
  if nullFreq freq then return Nothing
    ...
    let itemBase = buildItem flavour discoRev itemKindId itemKind lid jdamage
        ...
        itemFull = ItemFull {..}
    return $ Just ( (kindIx, jdamage, jsource itemBase, aspectRecord)
                  , itemFull
                  , itemDiscoData
                  , seed
                  , itemGroup )
...
-- All the other meaningul properties can be derived from the item kind index,
-- the damage dice, item source and the aspects.
type ItemKnown = (ItemKindIx, Dice.Dice, ItemSource, AspectRecord)
