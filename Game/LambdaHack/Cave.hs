    ...
    (dmap, secretMap) <- do
      let f (l, le) ((x, y), t) =
                  case t of
                        ...
                        if not rb
                          then return (l, le)
                               ...
                               then return (M.insert (x, y) doorOpenId l, le)
                                 ...
                                 if not rsc
                                   then return (M.insert (x, y) doorClosedId l,
                                                le)
                                   else do
                    ...
                    _ -> return (l, le)
      foldM f (lm, M.empty) (M.toList lm)
    let cave = Cave
          { dkind = ci
          , dsecret = secretMap
          , ditem = M.empty
...
mergeCorridor :: Kind.Id TileKind -> Kind.Ops TileKind -> Kind.Id TileKind -> Kind.Id TileKind -> Kind.Id TileKind
