...
The WASM test suite drives the compiled wasm test binary through Node/wasmtime via `ts-src/run-wasm-test.mjs`; the TypeScript tests run vitest over `ts-src/`. Both run in CI, one job each in `lint-and-playtest.yml`; the wasm job installs the toolchain from scratch, so locally they need `~/.ghc-wasm/env` (sandboxing notes at the end of this file):
