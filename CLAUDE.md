...
- `.github/workflows/haskell-ci.yml` is generated by `haskell-ci` from the `.cabal` file — regenerate it (`haskell-ci regenerate`) rather than hand-editing. Anything the generator does not produce belongs in `cabal.haskell-ci`, not in the generated file: `apt:` for extra system packages, `github-patches:` for a diff reapplied on every regeneration. A hand edit left outside that config is not merely fragile, it disappears *silently* on the next regenerate — which is how this repo nearly lost `libsdl2-ttf-dev` from its apt line. After changing the config, regenerate twice and compare: a patch that applies cleanly once can double-apply. (In horde-ad the same role is played by the `tests` and `benchmarks` steps, marked by a comment in the workflow.)
