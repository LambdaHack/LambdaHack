...
Frontend work is planned in `docs/wasm-frontend-unified-plan.md`: the SDL2-parity roadmap for the WASM frontend, recorded decisions — including don't-do rulings, so ideas aren't re-proposed — and verified repo facts with file:line citations. Consult it before frontend-touching changes. It is kept indefinitely but is not frozen yet, so it is neither of the two kinds below: its ledger gives every item a state, an unlanded item is live specification and a landed one is frozen history, and a landing appends an outcome line rather than rewriting the item — so the file matures into a record instead of being deleted. In a live item a drifted claim is an error to fix; in a landed one it is left alone, and only a claim that *resolves* earns a further outcome line.
...
- The wrapper mounts a hand-picked, changeable subset of HOME, curated per-path rather than per-directory: that an entry is visible says nothing about how much of its contents is (at times `~/r` has held only the current repo, and `~/.ssh` carries public material only). Don't infer access from a directory listing and don't record mount inventories — they go stale; verify the specific path at the moment it matters.
  - `~/.ghc-wasm` is the standing example, and it has been mounted, absent and half-mounted in turn, so probe it rather than believe any of those: `. ~/.ghc-wasm/env && command -v wasm32-wasi-cabal`. The presence of `env` proves nothing — the file has been visible with the toolchain behind it hidden, in which case sourcing succeeds, the command still does not resolve, and the three targets that source it (`make build-wasm`/`build-ts`/`test-wasm`) fail well past the point that names the cause. Ask for the mount when the probe fails. `build-ts` additionally writes into `../lambdahack.github.io`, so it needs unsandboxed Bash even when the toolchain is there.
