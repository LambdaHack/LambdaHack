...
stylish-haskell (`.stylish-haskell.yaml`; the editor runs it on save).
The `arguments:` in`.hlint.yaml` pass `--cpp-simple` (keeps the
...
- Always: `python3 tools/check-plan-citations.py DOC` over CLAUDE.md, the
  `docs/` document and every draft staged in the repo root; then,
...
- When Haskell code changed: build, then `cabal test` (the Makefile
  ...
  outside the editor misses its on-save stylish pass, so run them by hand).
  Hlint should report no suggestions and no errors when run with
  `--ignore-glob "engine-src/Game/LambdaHack/Common/JSFile.hs"`
  (unsupported `JavaScriptFFI` extension).
...
- The pointman is denormalized: the authoritative `sleader` lives in client
  ...
  (`FrameM.hs:155`), so a leader value held across an interactive wait can
  go stale — the root of the assertion disabled in commit 4a6eca154. Don't
  cache the pointman across a `promptGetKey` call; re-read `sleader` at
  the point of use.
...
Planning/reference documents (this file, the `docs/` document listed at
the top and any GitHub-bound drafts staged in the repo root) make
checkable claims. Whenever such a document (or code it cites) is edited,
run three passes:
...
1. `python3 tools/check-plan-citations.py [DOC]` (from the repo root) —
   ...
   `docs/wasm-frontend-unified-plan.md` to check that one.
