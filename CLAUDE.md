...
File:line references were verified against the tree at commit `2815391d4` (2026-07-28); the citation pass proves a cited line exists — this stamp, that it still says what the claim around it needs.
...
A second reference document, `docs/leader-desync-bug.md`, covers UI client state: the post-mortem of the pointman-desync bug family (the crash worked around in commit 4a6eca154), the de-risked live-read fix design — make `sleader` the single source of truth, read at point of use — and the test battery pinning it. Its companion, `docs/promptgetkey-hygiene.md`, holds the abort-split design — strictly after live-read and assuming it, split `promptGetKey`'s interrupted-macro cleanup into a pure decision plus a named abort action — plus everything joint to the two designs, notably the [contract] test series that must stay green across both refactors. The dependency is one-way by design: the post-mortem never references the abort-split design or its document, so don't "fix" that by adding a link. Consult them before touching pointman/leader plumbing, macro playback, run continuation or the item-dialog code.
...
- **Where a sibling checkout and the released version it tracks disagree, the checkout wins.** It is ahead of the release and actively worked on, not stale, so read it for what upstream *is* (in horde-ad: `../orthotope` sat at 0.1.8.0 plus unreleased commits rewriting the very strided fallback a performance note described, which would have been recorded as current had the release been treated as the authority). The cabal store answers the two questions the checkout cannot: what to read when the wrapper hides the mount, and whether a *version-sensitive name* a document pins is really in the release the build plan resolves. A note resting on upstream code the checkout has since changed says so, and names what to re-measure when that work lands.
