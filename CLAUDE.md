...
- Always, over every `.md` document in the repo (`git ls-files '*.md'` plus untracked drafts, minus `CHANGELOG.md`, a historical record rather than a live claim set):
  ...
  - re-stamping, for a document whose stamp names a commit the citations were not verified against — which is every replayed branch, and had gone stale four times before the flag existed: `python3 tools/check-plan-citations.py DOC --restamp` rewrites the stamp to name the newest commit touching a cited file — not HEAD, so that amending or replaying the commit that carries the document cannot falsify it — and only on a clean pass with the cited files unmodified, so a refusal is information. It cannot know you did the *reading* the stamp also asserts, so pass it for a document you have just re-read and not for one you merely ran the checkers over;
