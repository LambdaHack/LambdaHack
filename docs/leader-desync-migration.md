...
> lists in full. Re-run that command rather than trusting the numeral, which has
> read eight and then nine and was wrong both times. When sec. 04's last step
> is done:
>
> - delete this file, and `tools/leader-census.py` with it --- except
>   that the post-mortem's sec. 10.3 rests its census-rather-than-floor claim
...
>   and add to each record the outcome line it reserves. **? Open,
>   and the author's to close:** does this commit add all of them, or does PR 0
>   append the two it resolves when it resolves them? PR 0 pins the apply dialog
>   and lands the AS cases, which is what closes the post-mortem's
>   "the apply-dialog one by nothing" and the abort-split record's "two
>   are unpinned". *This commit*: one commit touches the records, and the status
>   quo needs no decision. *PR 0*: the records stop under-claiming their own
>   coverage the moment it lands, rather than staying wrong for two PRs,
>   at the cost of a second commit into a frozen record --- which is permitted,
>   an outcome line per resolved claim being exactly the upkeep those records
>   reserve, and not the forbidden updating of a post-mortem to match a later
>   tree. Answer it before PR 0 lands, not here: this bullet is where it fires,
>   PR 0 is where it binds, and if PR 0 goes first unanswered the default has
>   been taken by omission;
...
>   is read, never driven to zero, since sec. 11 goes on saying "the migration"
...
> - and the six sites outside the pointman records, which are the ones a reader
>   forgets: `docs/wasm-frontend-unified-plan.md` names this file four times ---
>   the tag-ownership sentence, which keeps its claim and drops only
...
lives in the repository rather than in someone's head. Steps 2 to 5 of sec. 02
...
from the repo root. Read the counts and the printed snippets, not the exit
status alone --- a suite that silently loses a test still passes, and a citation
that slid still resolves:
...
python3 tools/leader-census.py                   # before step 2 only; sec. 03
python3 tools/check-plan-citations.py <this file, the post-mortem, the plan>
```

Those three counts move at exactly two points and nowhere else. PR 0 of sec. 01
takes them to 157, 28 and 11 --- two AS cases and one flip pin --- and sec. 02
step 4 then deletes LR6, taking them to 156, 28 and 10. A "count unmoved"
in the table above is against whichever of the three baselines its row follows,
...
and `HandleHelperM.hs` lines the same conversions slide --- cited there,
not written, so no **Owns** covers them and a frozen appendix restamps green
while pointing at the wrong lines. `--restamp` then follows the reading pass,
as the header says.
...
versus witness-free (the post-mortem's sec. 10.4); any `[contract]` test whose
*outcome* will not come back green --- a call site that merely needs a witness
to compile is not one of those, and sec. 02 step 4 names the seven such sites;
...
`projectItem` or `meleeAid`, which the post-mortem's sec. 10.6 pins; don't
convert the nineteen keep-param entries of sec. 03's tail; don't fix the apply
closure the way `psuitReq`'s is fixed, by moving the call, since there it
is the *actor* that must go live; and don't bind a `getLeaderUI` result before
a `promptGetKey` and use it after, which is this whole document in one sentence.
...
or classification here found wrong, an ordering constraint discovered, a **?
Open** question closed. Not log-worthy: doing a step as written, or editing
this file before the work starts --- an entry that records only that the plan
was written is one the next reader has to skip.
...
| 1 -- live-read | sec. 02 steps 1--5, in two commits: step 1, then steps 2 to 5 together | the flips, the new apply one included, land with the engine change that earns them |
...
> the rest of the family produces --- and that cuts both ways, which is why
> it is recorded here rather than acted on. *For* reordering: a crash a player
> can reach argues for landing live-read first, since PR 0 fixes nothing and PR
> 1 is what closes it. *Against*: PR 0 is what makes PR 1 checkable, sibling (c)
> has no test at all, and the campaign has already been wrong twice in ways only
> a test caught --- reordering saves little and spends the safety net exactly
> where this work has needed it. Weighing against hurry too: forcing the bad
> thunk needs more than one item selected, or the ground store, since every
> other path drops it unforced or guards it (sec. 03), which is why the years
> this code has stood have produced no report. Until this is answered the table
> above stands.
...
> sec. 04.4's gate can enumerate it, and the two are selectable in isolation.
> *Added assertion*: the series stays AS1--AS14, AS4's own comment records what
> it now observes, and the count moves by one rather than two --- which changes
> PR 0's arithmetic above, so close this before quoting those numbers anywhere.
> Until it is answered, sec. 04.4's row says "the whole AS series" and names
> no numbers, deliberately: naming them would foreclose the second branch.
...
and step 4's flips are not all earned by step 2, seven of the ten turning
on conversions step 5 performs. Step 1 lands before them, its own commit
and green on its own, the accessors having no callers yet. The abort-split
(sec. 04) is a separate change on top; the test battery (sec. 05) is a separate
one below, and has already landed.
...
4. **Flip the whole [LR-flip] series**, verifying each flip by temporarily
   ...
   tests, eleven once PR 0's apply-dialog pin lands --- and each states
   its target value inline (the set's membership changed on 2026-07-30 without
   its size doing so, which is why the count is worth re-reading rather
   than trusting):
   ...
   and the suite 157 -> 156, the second of the two movements sec. 00's count
...
5. **Sweep what step 2 left**, in two halves, because only one of them
   ...
   parameters and sec. 03's other nineteen alone --- the post-mortem's sec. 10
...
6. **Verification**: the full unit suite (154 today, 157 once PR 0 has landed
   ...
   than only at its end --- that series is the safety net the conversion runs
   on; `hlint .`; `make test-short` / `test-medium` playtests (AI-driven ---
   they exercise the client loop, not the dialogs --- and minutes each,
   so budget for them rather than reading one as a hang); a manual session
   replaying the post-mortem's sec. 04 timeline (multi-hero run inside
   a recorded macro that opens the item menu, then `A-Tab`/`C-Tab`) --- X1
   ...
   is the judgement and the sequencing, not the typing: no run has yet reached
   the sec. 04 window itself, and whether `A-Tab` survives inside a menu
   is unsettled. Performance needs no gate --- the post-mortem's sec. 11:
...
**The pinned-site comments**, which step 5 owes the post-mortem's sec. 10.6 ---
...
then one per code-carrying row of sec. 00 bar the spike, which either reverts
or becomes step 1 --- so the history reads as the plan does and no step
is tempted to bundle. The bodies are written from what the step actually did ---
that part cannot be drafted in advance --- but the titles can, and they fix
...
Add the pointman witness and the live-read accessors      (sec. 02.1)
Read the pointman live in the item dialogs                (sec. 02.2-4)
Stop threading the pointman through the command boundary  (sec. 02.5)
...
the post-mortem's sec. 10.6 overrides the "holds the pointman across a wait"
test rather than being served by it. `projectItem` (`HandleHumanGlobalM.hs:967`)
...
sec. 01's PR 0 is where it belongs, alongside the two AS cases --- sec. 02 step
